...
function downloadBlob(filename: string, mime: string, data: Blob | string) {
  ...
  const url = URL.createObjectURL(blob); // 標準的なダウンロード手法（a[download]と併用）。 :contentReference[oaicite:1]{index=1}
...
async function fetchJPFontB64() {
  ...
  for (const u of JP_FONT_URLS) {
    try {
      const ab = await fetch(u, { mode: 'cors' }).then(r => { if (!r.ok) throw new Error(String(r.status)); return r.arrayBuffer(); });
...
// #isCapturing 内の <img> を data: 化して収集（CORS 不可は除外）
async function collectDataImagesFromIsCapturing(rootEl: HTMLElement) {
  ...
  for (const img of imgs) {
    ...
    // 2) crossOrigin + canvas（tainted の場合は失敗）
        try {
          const i = new window.Image();
          i.crossOrigin = 'anonymous'; (i as any).referrerPolicy = 'no-referrer'; i.decoding = 'async';
...
// 要約：バックエンドがあれば利用、なければ #isCapturing の Markdown
async function getSummaryFromBackendOrLocal(rootEl: HTMLElement) {
  try {
    ...
    if (r.ok) { const j = await r.json(); if (j && typeof j.summary === 'string') return j.summary; }
  } catch {}
...
export default function Home() {
  ...
  const [totalShots, setTotalShots] = useState<number>(() => Number(localStorage.getItem('totalShots') || '0'));
  const [totalSize, setTotalSize] = useState<number>(() => Number(localStorage.getItem('totalSize') || '0'));
  const [deletedCount, setDeletedCount] = useState<number>(() => Number(localStorage.getItem('deletedCount') || '0'));
    ...
    (async () => {
      try {
        if (window.api?.listScreenshots) {
          ...
          if (mounted && Array.isArray(imgs)) setPreviewImages(imgs.slice(0, 4));
        }
      } catch {}
    })();
    return () => { mounted = false; };
  ...
  // ダーク状態の適用
  useEffect(() => {
    try { const dark = localStorage.getItem('darkMode') === '1'; document.body.classList.toggle('dark', !!dark); } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      ...
      timer = setTimeout(async () => {
        try { if (window.api?.writeSettings) await window.api.writeSettings({ interval: intervalMin, resolution: parseFloat(resolution), statusText, isRecording }); } catch {}
      }, 500);
    };
    schedule();
  ...
  const onGenerateReport = useCallback(async () => {
    ...
    const fmt = (fmtSel?.value || 'pdf').toLowerCase();
    ...
    try {
      ...
      if (items.length === 0 && root) items.push(await snapshotIsCapturing(root)); // 最低1枚
      ...
      const buildMarkdown = (p: typeof pack) => {
        const L: string[] = [];
        L.push(`# ${p.title}`, '', `生成時刻: ${new Date().toLocaleString()}`, '');
        if (p.includeSummary) L.push('## 要約', '', p.summary || '（要約なし）');
        return L.join('\n');
      };
      const buildPlainText = (p: typeof pack) => {
        const L: string[] = [];
        L.push(`${p.title}`, `生成時刻: ${new Date().toLocaleString()}`, '');
        if (p.includeSummary) L.push('【要約】', p.summary || '（要約なし）');
        return L.join('\n');
...
<h1>${esc(p.title)}</h1><div class="meta">生成時刻: ${esc(new Date().toLocaleString())}</div>${summaryHTML}${imagesHTML}</html>`;
      ...
      // --- md / txt も復活 ---
      if (fmt === 'markdown') return downloadBlob(`${title}.md`, 'text/markdown;charset=utf-8', buildMarkdown(pack)); // text/markdown は RFC 7763 で登録。 
      ...
      if (fmt === 'docx') {
        ...
        return downloadBlob(`${title}.docx`, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', blob);
      }
      ...
      if (fmt === 'pdf') {
        ...
        const tryVector = async (): Promise<Blob | null> => {
          ...
          const ok = await attachJPFont(doc); if (!ok) return null;
          const page = { w: doc.internal.pageSize.getWidth(), h: doc.internal.pageSize.getHeight(), margin: 40 };
          const maxW = page.w - page.margin * 2, lineH = 14;
          ...
          if (pack.includeSummary) {
            ...
            doc.setFontSize(11); y = renderPDFMultilineText(doc, pack.summary || '（要約なし）', page.margin, y, maxW, lineH); y += 6;
          }
          for (let i = 0; i < pack.items.length; i++) {
            const img = pack.items[i];
            if (!img.dataURL.startsWith('data:')) continue;
        ...
        let blob = await tryVector(); if (!blob) blob = await tryRaster();
      ...
      pushActivity(`レポート生成(${fmt.toUpperCase()})`);
    }
  }, [intervalMin, resolution, statusText, isRecording]);

  // ======= ここから UI =======
  if (status === 'loading') return <main><p>セッション情報を読み込み中です...</p></main>;
          ...
          <div className="grid" style={{ display: 'flex', gap: 20 }}>
                  ...
                  <div className="card-content">
                    <div className="control-section">
                      ...
                      <div className="form-group controls-row" style={{ display: 'flex', gap: 12 }}>
                        {/* 単一ボタン：開始↔停止 */}
                        <button
                          onClick={async () => { try { if (!isRecording) await handleStart(); else await handleStop(); } catch(e){} }}
            ...
            <div className="col-right" style={{ flex: '1 1 32%' }}>
                ...
                <div className="card-content">
                  <div className="report-section">
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <label className="toggle-switch">
                        <input
                          type="checkbox" id="deleteAfterUploadToggle" checked={deleteAfterUpload}
                          onChange={(e) => { const v = e.target.checked; setDeleteAfterUpload(v); try { localStorage.setItem('deleteAfterUpload', v ? '1' : '0'); } catch {} }} />
                        <span className="slider"></span>
                      </label>
                    </div>
              ...
              <section className="card">
                ...
                <div className="card-content"><div className="report-list" id="reportList"></div></div>
              </section>
            </div>
          </div>
        </div>
